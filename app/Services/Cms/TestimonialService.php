    ...
    public function data()
    {
        ...
        return DataTables::of($query)
            ->addColumn('actions', function ($item) {
                return '
                    <div class="btn-group">
                        <a href="' . route('admin.cms.testimonials.edit', $item) . '" class="btn btn-outline-warning "><i class="fa fa-edit"></i></a>
                        <a href="' . route('admin.cms.testimonials.destroy', $item) . '" class="btn btn-outline-danger btn-delete"><i class="fa fa-trash"></i></a>
                    </div>
                ';
            })
            ->editColumn('customer_photo', function ($item) {
                return $item->customer_photo ? '<img src="' . asset($item->customer_photo) . '" width="100" height="100">' : '-';
            })
            ->rawColumns(['actions', 'customer_photo'])
            ->make(true);
    }
}
